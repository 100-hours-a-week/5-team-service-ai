...
class DiscussionSummaryService:
    ...
    def _build_prompt(self, *, topic: str, round_no: int, context: str) -> str:
        ...
        return (
            "너는 한국어 토론 요약 어시스턴트다.\n"
            "다음 채팅 메시지를 보고 긍정/우려, 핵심 쟁점, 미해결 쟁점을 간결히 정리해라. 모든 문장은 중립적 보고체(회의록 톤, ~함/됨/임)로 작성하라.\n"
            ...
            f"대화 로그:\n{context}\n"
            "규칙: JSON 외 텍스트/설명/코드펜스 금지. 각 항목은 한국어 한 문장으로 1~3개, 군더더기 없이 매우 간결하게 요약. 모든 문장은 중립 보고체를 유지.\n"
